...
from app.admin.views import AdminRoles
...
class MyAuthProvider(AuthProvider):
        ...
        if request.method == "GET":
            ...
            return admin.templates.TemplateResponse(
                "login.html",
                {
                    ...
                    "redirect_url": config.webhook.DOMAIN + config.admin.BASE_URL,
                },
            )
    ...
    async def login(
        ...
        if auth_data.id in [config.bot.ADMIN_ID, config.bot.DEV_ID]:
            ...
            admin = await Admin.get(request.state.db_session, user_id=auth_data.id)
    ...
    async def is_authenticated(self, request) -> bool:
        ...
        if (
                ...
                await Admin.is_exists(request.state.db_session, user_id=admin_id)
        ):
            request.state.user = request.session
            return True
        return False

    def get_admin_user(self, request) -> MyAdminUser:
        """
        Get the admin user.
        ...
        if admin_id not in [config.bot.ADMIN_ID, config.bot.DEV_ID]:
            async def get_admin():
                admin = await Admin.get(request.state.db_session, user_id=admin_id)
                request.state.user["roles"] = admin.roles

            asyncio.create_task(get_admin())

        return MyAdminUser(**request.state.user)
